...
- поддерживает glossary для терминов, имён, сленга и служебных токенов: case-insensitive сопоставление, регистр target-слова подстраивается под исходный (`Соло`, `соло`, `СОЛО` — всё из одной записи);
...
Glossary поддерживает case-insensitive сопоставление, поэтому для одного термина достаточно одной записи — регистр подстраивается автоматически. Например, запись `"Solo" -> "Соло"` будет корректно работать и для `"solo"`, и для `"SOLO"`.
...
Ключ в `.env` и ключ в конфиге `night-city-2045` не совпадают.
